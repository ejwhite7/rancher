...
- Public routes query Prismic’s published ref and use Prismic’s actual first-publication timestamp for live dates. A provisional date never hides a document that an editor has published. Related links and curated cards appear only when their targets are available in the same collection. Previews use the actual draft ref and noindex/no-store headers.

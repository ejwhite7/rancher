...
- Public routes require both a published Prismic document and a `published_at` date at or before the current time. Publishing a future-dated document early in Prismic does not expose it on the site. Lists, article routes, author listings, related links and the sitemap share this filter. A missing date falls back to Prismic’s first-publication timestamp. `legacy-publication-dates.json` corrects only the exact stale provisional dates on the three original imports; subsequent date edits are honored. Related links and curated cards appear only when their targets are available in the same collection. Previews use the actual draft ref and noindex/no-store headers.

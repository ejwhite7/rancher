...
6. After a confirmed save, PostHog identifies the **referrer** by normalized email with their name and email domain, then captures `referral_form_submitted` with `form`, `submission_id`, both submitted names and emails, `company_size`, and `industry`. An active PostHog Workflow forwards those event fields to `#form-submissions` in Slack. The referred person's identity is not written to the referrer's PostHog person profile. Analytics failure never reverses the saved confirmation.
